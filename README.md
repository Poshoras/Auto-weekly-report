...
# 记录一下如何利用通信协议在工作流A中访问工作流B
Case3由于是访问一个Agent，本质上不需要对参数进行严格的限制，但是我访问refine的那个工作流，是有很多参数的，直接访问url，电脑是操作不了的，需要人手动输入两个参数的具体内容。

# 但最后并没有用文件，因为文件联网，无法访问


于是不得不使用MCP通讯协议，先在插件MCP-server中将旧工作流封装成为新的工具，再从Agent-functioncalling里面去调用这个插件，才能成功实现访问，真的是麻烦死了，妈的。

但最简单的还是将工作流直接封装为工具。

想到了一个点，分类：1.数字类 2.自然语言类。数字类自不用多说，自然语言类就使用过去八周的PPT的文字。
